...
class D_GET_LOGITS(nn.Module):
    def __init__(self, ndf, nef, bcondition=True):
        ...
        if bcondition:
            self.outlogits = nn.Sequential(
                ...
                nn.Conv2d(ndf * 8, 1, kernel_size=2, stride=4),
                nn.Sigmoid())
        else:
            self.outlogits = nn.Sequential(
                nn.Conv2d(ndf * 8, 1, kernel_size=2, stride=4),  # change kernel 4 -> 2
                nn.Sigmoid())
    ...
    def forward(self, h_code, c_code=None):
        ...
        if self.bcondition and c_code is not None:
            ...
            c_code = c_code.repeat(1, 1, 2, 2)  # CHANGE 4 -> 2
            #print('-'*50)
            #print(c_code.size())
            #print(h_code.size())
            # state size (ngf+egf) x 4 x 4
            #  size 128, 128, 2, 2 c code
...
class STAGE1_G(nn.Module):
    ...
    def define_module(self):
        ...
        self.fc = nn.Sequential(
            nn.Linear(ninput, ngf * 4, bias=False),  # change
            nn.BatchNorm1d(ngf * 4),  # change
            nn.ReLU(True))
    ...
    def forward(self, text_embedding, noise):
        ...
        h_code = h_code.view(-1, self.gf_dim, 2, 2)  # remove one 4 or 2 x 2  CHANGE
...
class STAGE1_D(nn.Module):
    ...
    def define_module(self):
        ...
        self.encode_img = nn.Sequential(
            ...
            nn.Conv2d(ndf*4, ndf * 8, 3, 3, 1, bias=False),  # CHANGE filter 4 -> 3
            nn.BatchNorm2d(ndf * 8),
...
class Detectron():
    ...
    def arr_to_rgb(self, img):
        ...
        red_arr_b = 255.0 * rescale(img[0,:,:])
        green_arr_b = 255.0 * rescale(img[1,:,:])
        blue_arr_b = 255.0 * rescale(img[2,:,:])
        img[0, :, :] = red_arr_b
        img[1, :, :] = green_arr_b
        img[2, :, :] = blue_arr_b
