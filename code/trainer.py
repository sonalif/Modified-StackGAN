...
def get_ohe(labels):
    labels_ohe = []
    for label in labels:
        result = [id for id, x in enumerate(classes) if x in label]
        det_ohe = np.zeros(len(classes))
        det_ohe[result] = 1
        labels_ohe.append(det_ohe)

    return torch.FloatTensor(labels_ohe)
